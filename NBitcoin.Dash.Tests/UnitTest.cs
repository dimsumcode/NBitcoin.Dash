using NBitcoin.DataEncoders;
using System;
using NUnit.Framework;
using System.Threading;
using NBitcoin.Protocol;

namespace NBitcoin.Dash.Tests
{
    [TestFixture]
    public class UnitTest1
    {
        [Test]
        public void X11PoWTest()
        {
            {
                NBitcoin.Dash.Networks.Register();
                CancellationTokenSource cts = new CancellationTokenSource();
                var client = Node.Connect(NBitcoin.Dash.Networks.Testnet, "127.0.0.1:19999", new NodeConnectionParameters()
                {
                    Version = (ProtocolVersion)70208,
                    ConnectCancellation = cts.Token
                });

                Assert.True(client.IsConnected);

                var chain = client.GetChain();
            }
        }

        [Test]
        public void Test1()
        {
            Networks.Register();
            BitcoinAddress.Create("XpESxaUmonkq8RaLLp46Brx2K39ggQe226", Networks.Mainnet);
            BitcoinAddress.Create("Xk3pg1LTNYH6djvJjrn4bV68WRnRQDyqsr", Networks.Mainnet);
            BitcoinAddress.Create("yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA", Networks.Testnet);

            Assert.Throws<FormatException>(() => BitcoinAddress.Create("yPv7h2i8a3dJjfSH4L3x91JSJszjdbsJJA", Networks.Testnet));
            Assert.Throws<FormatException>(() => BitcoinAddress.Create("yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA", Networks.Mainnet));

            Assert.AreEqual(Networks.Testnet, BitcoinAddress.Create("yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA").Network);
            Assert.AreEqual(Networks.Mainnet, BitcoinAddress.Create("XpESxaUmonkq8RaLLp46Brx2K39ggQe226").Network);

            var header = new BlockHeader("01000000f615f7ce3b4fc6b8f61e8f89aedb1d0852507650533a9e3b10b9bbcc30639f279fcaa86746e1ef52d3edb3c4ad8259920d509bd073605c9bf1d59983752a6b06b817bb4ea78e011d012d59d4");
            Assert.AreEqual("adf6e2e56df692822f5e064a8b6404a05d67cccd64bc90f57f65b46805e9a54b", header.ToString());

            Assert.AreEqual("0000000110c8357966576df46f3b802ca897deb7ad18b12f1c24ecff6386ebd9", Networks.Mainnet.Consensus.GetPoWHash(header).ToString());

            var b = new Block(Encoders.Hex.DecodeData(""));
            Assert.True(b.CheckMerkleRoot());
            Assert.True(b.Header.CheckProofOfWork(Networks.Mainnet.Consensus));

            var genesis = Networks.Mainnet.GetGenesis();
            var b1 = new Block(Encoders.Hex.DecodeData(""));

            var b2 = new Block(Encoders.Hex.DecodeData(""));

            var chain = new ConcurrentChain(Networks.Mainnet);
            chain.SetTip(b1.Header);
            chain.SetTip(b2.Header);
            Assert.AreEqual(2, chain.Height);
            Assert.True(chain.Validate(Networks.Mainnet));
        }
    }
}

